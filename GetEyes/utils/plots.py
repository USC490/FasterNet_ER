...
# from utils.general1 import xywh2xyxy, xyxy2xywh
# from utils.metrics import fitness
from detect_face.utils.general1 import xywh2xyxy, xyxy2xywh
from GetEyes.utils.metrics import fitness
...
def plot_one_box(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    if label:
        if len(label.split(' ')) > 1:
            ...
            cv2.putText(im, label, (c1[0], c1[1] - 2), 0, tl / 6, [225, 255, 255], thickness=tf//2, lineType=cv2.LINE_AA)
    if kpt_label:
        rec =plot_skeleton_kpts(im, kpts, steps, orig_shape=orig_shape)
        return rec
...
def plot_skeleton_kpts(im, kpts, steps, orig_shape=None):
    ...
    for kid in range(num_kpts):
        ...
        if not (x_coord % 640 == 0 or y_coord % 640 == 0):
            ...
            points[kid][0] = x_coord
            points[kid][1] = y_coord
            # 左右眼的距离
    return points
    # leftToRightDist = int(
    ...
    # cv2.circle(im, (leftEyeLX, leftEyeLY), 4, (0, 0, 0), -1)
    #
    # # 眼睛正方形框截取
    ...
    # cv2.circle(im, (leftEyeRX, leftEyeRY), 4, (0, 0, 0), -1)
    #
    ...
    # cv2.circle(im, (rightEyeLX, rightEyeLY), 4, (0, 0, 0), -1)
    #
    ...
    # cv2.circle(im, (rightEyeRX, rightEyeRY), 4, (0, 0, 0), -1)
    #
    # rightEyeRY1 = min(imageH, int(points[1][1] + leftToRightDist / 3))
    # # print('右眼下坐标：', rightEyeRX, rightEyeRY)
    #
    # # 抠出左眼框图像数据
    # # leftEyeImage = images[leftEyeLY:leftEyeRY, leftEyeLX:leftEyeRX, :]
    # leftEyeImage = im[leftEyeLY:leftEyeRY, leftEyeLX:leftEyeRX, :]
    #
    # leftEyeImage1 = im[leftEyeLY1:leftEyeRY1, leftEyeLX:leftEyeRX, :]
    ...
    # lrightEyeImage = im[rightEyeLY:rightEyeRY, rightEyeLX:rightEyeRX, :]
    # lrightEyeImage1 = im[rightEyeLY1:rightEyeRY1, rightEyeLX:rightEyeRX, :]
    #
    # # eyesImage = im[leftEyeLY:rightEyeRY, leftEyeLX:rightEyeRX, :]
    # # eyesImage1 = im[leftEyeLY:rightEyeRY1, leftEyeLX:rightEyeRX, :]
    # # cv2.imwrite(os.path.join(save_leftEye, leftEyePath + str(index) + x + '.jpg'), leftEyeImage)
    # # cv2.imwrite(os.path.join(save_rightEye, rightEyePath + str(index) + x + '.jpg'), lrightEyeImage)
    # # cv2.imwrite(os.path.join(save_twoEye, eyespath + str(img_num) + x + '.jpg', ), eyesImage)
    # #
    # # cv2.imwrite(os.path.join(save_leftEye1, leftEyePath + str(index) + x + '(1).jpg'), leftEyeImage1)
    # # cv2.imwrite(os.path.join(save_rightEye1, rightEyePath + str(index) + x + '(1).jpg'), lrightEyeImage1)
    # # cv2.imwrite(os.path.join(save_twoEye1, eyespath + str(img_num) + x + '(1).jpg', ), eyesImage1)
